...
## 2回目の調整（同日、参考デザインへの追従）

- 見出し（英字＋日本語）を左揃えに戻し、青・グレーの帯（実績数字・メッセージ・お問い合わせ）だけ中央揃え。
- 実績グリッドをフラット化（カードの枠・影をやめ、画像＋区分の小さなグレー文字＋太字タイトル）。「制作事例一覧を見る」を丸い矢印付きに。
- フッターを「左：会社情報／右：リンク3列」に（HTML は変えず footer の .wrap をグリッド化）。
- お問い合わせ帯のグレーを参考に近い濃さへ。ヘッダーのナビ文字間隔を広げ、丸型「お問い合わせ」を大きく。
- 検証：24ページのスナップショット差分は引き続き「#top｜TOP」の追加のみ。Lighthouse（モバイル）アクセシビリティ／SEO は /・/works/・/services/immersive/ で 100。ビルド・監査・テスト通過。

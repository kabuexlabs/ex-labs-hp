...
- **Tier ピラー（immersive / madamis / zunousen / shinrisen / shisetsu-katsuyo / taikengata-event / shuyu-event / botsunyukan / saiji）の凍結は 2026-09-21 で終了**。以後も title・h1・URL は原則変更しない（変更するなら理由・日付を docs/title-history.md と seo-log に残す）。本文改修は「確定した直近14日以上の GSC で順位が維持されている柱」に限り、GSC 10位以下の複合語への「答えの段落」を1回1〜2段落ずつ追加する。順位が下がっている柱は原因候補を記録して本文改修を見送る。ただし追記や内部リンクも順位に影響し得るので「追加なら無条件に安全」とは扱わず、目的のある変更に絞って docs/seo-log.md に記録する。
...
- 東京ガイド /guide/immersive-tokyo/ は「東京のイマーシブを比較して選ぶ」中心ページ（2026-09-15 改修）。比較表・公演カード・選び方は src/data/events/ から生成し、日程・料金を本文に手書きしない。他社公演は公式ページで確認できたものだけ published: true（未確認は候補として非公開のまま）。「東京 イマーシブ おすすめ／人気／一覧」の別ページは作らない。

...
// 情報は一切含めない。
export type MetricEvent = 'view' | 'cta' | 'submit' | 'ticket' | 'show' | 'ev-search' | 'ev-zero' | 'ev-view' | 'ev-ext';
// ticket＝外部チケットサイトへのクリック、show＝自社の公演詳細へのクリック（購入完了ではない）
// ev-search＝公演検索の利用（値は 日付種別-人数-エリア-ジャンル）、ev-zero＝検索結果ゼロ、ev-view＝公演詳細の閲覧（値は 作品-own|3rd）、ev-ext＝外部サイトへのクリック（値は サイトID-掲載箇所）
export const METRIC_EVENTS: MetricEvent[] = ['view', 'cta', 'submit', 'ticket', 'show', 'ev-search', 'ev-zero', 'ev-view', 'ev-ext'];
